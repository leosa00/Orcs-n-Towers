...
public:
    MissileProjectile(sf::Vector2f position, int damage, std::shared_ptr<Enemy>&& targetEnemy) //modified std::shared_ptr<Enemy>& targetEnemy to std::shared_ptr<Enemy>&& targetEnemy (std::shared_ptr<Enemy>& targetEnemy was giving error in shoot method)
